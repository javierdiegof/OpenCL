__kernel void idcheck(__global float* output){
	...
	size_t local_size_0 = get_local_size(0);
	size_t local_size_1 = get_local_size(1);
	...
	size_t index1 = global_id_1 - global_offset_1;
	// Se hace el mapeo del valor
	size_t index = index1*global_size_0 + index0;
	
	// Obtener el indice final
	output[index] = global_id_0*0.1 + global_id_1;
}