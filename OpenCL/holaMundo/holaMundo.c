...
int main(){
	...
	// Crear buffer
	char cadena[20];
	cl_mem d_cadena = clCreateBuffer(context,CL_MEM_WRITE_ONLY,sizeof(cadena),NULL,&err);
	if(err < 0) error("Error al crear el buffer\n");
	
	
	// Asignar d_Cadena como argumento
	err = clSetKernelArg(kernel,0,sizeof(d_cadena), &d_cadena);
	if(err < 0) error("Error al asignar a la cadena como argumento del Kernel\n");
	
	// Ejecutar el kernel hello(d_cadena)
	err = clEnqueueTask(command_queue, kernel, 0, NULL, NULL);
	if (err < 0) error("Error al ejecutar el kernel");
	
	// Leer el buffer del dispositivo
	err = clEnqueueReadBuffer(command_queue, d_cadena, CL_TRUE, 0, 20, cadena,0, NULL, NULL);
	if(err < 0) error("Error al leer el buffer del dispositivo \n");
