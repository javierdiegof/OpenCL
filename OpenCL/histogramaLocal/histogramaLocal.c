...
int main(){
    ...
    FILE *archivo;
    archivo = fopen("./histogramaLocal.cl", "r");
	...
	// Crear kernel
    cl_kernel kernel = clCreateKernel(program, "histogramal", &err);
	...
	err = clSetKernelArg(kernel, 2, sizeof(histo), NULL);
	if(err < 0) error("Error al asignar el kernel a la memoria privada. \n");
	err = clSetKernelArg(kernel, 3, sizeof(N), &N);
	if(err < 0) error("Error al asignar el kernel de N.\n");
	
	// Encolar kernel 
	size_t dim = 1;
	size_t global_size[] = {TAM_HISTO*15};
	size_t local_size[] = {TAM_HISTO};											
	size_t offset[] = {0};
